...
所有环境每行输出一个 ECS JSON 事件，dev 与 prod 使用相同 schema。ECS 已定义的字段使用 ECS 名；技术追踪关联使用 `trace_id` / `span_id`；平台字段使用 snake_case。禁止创建同义别名，例如用 `requestId`、`correlation_id`、`traceId` 重复表示 Trace，或用 `elapsed`、`retry_times`、`outcome` 替代标准字段。有独立业务含义的关联标识按第 3 节处理，不属于 Trace 的同义别名。
...
|---|---|---|---|
| `trace_id` | string，32 位非全零 hex | 执行单元内所有应用日志；当前 Trace 查询键 | 上下文 Adapter / OTel |
| `span_id` | string，16 位非全零 hex | 执行单元内所有应用日志；只用于与 Trace 对位 | 上下文 Adapter / OTel |
| `correlation_id` | string，有效非空关联；新根入口生成 UUID v4 | 已建立关联的执行单元内自动携带；查询根操作及其派生工作 | 入口 / 上下文 Adapter |
| `tenant_id` | string | 可信租户已建立时出现 | 身份 / 租户组件 |
| `actor_type` / `actor_id` | string；type 为 USER / SERVICE / SYSTEM | 当前可信 Actor；ID 不承担原始发起人语义 | 身份组件 |
| `initiator_type` / `initiator_id` | string；type 为 USER / SERVICE / SYSTEM | 原始可信 Initiator；不代替当前 Actor 授权 | 身份组件 |
| `user_id` | string | 仅当前 Actor 为 USER 时投影其 ID；SERVICE/SYSTEM 不填入 | 身份组件 |
| `device_id` / `client_id` | 类型由领域统一登记 | 入口可信解析到对应身份时出现；日志只投影，不另建来源 | 设备 / 客户端身份组件 |
...
| `duration_ms` | number，`>= 0` | 所有计时结果事件必有；用单调时钟差值转为毫秒，不用 UTC 时间戳相减 | 场景 Adapter |
...
- 同一条 Trace 内查询 `trace_id`；查看某个服务时查询 `trace_id + service.name`。一项业务操作可以跨多条 Trace，跨 Trace 查询使用该操作的业务关联标识或领域键。`span_id` 不作为日志执行单元 ID。
- 入口能可信解析到什么身份就注入什么，并在执行单元内自动携带；日志只投影已注入的身份，业务代码不重复传递。

业务关联与 Trace 的职责：

- Trace 标识执行追踪；Correlation ID 标识一次根入口操作及其因果派生工作。二者不要求值相等，也不要求生命周期一致。
- 根入口由基础设施生成独立 correlationId，内部 HTTP、异步任务与消息消费继承已有关联。一个入口派生多个命令可以共享关联，各命令保留独立身份；不将 `command_id` 或 Trace ID 兼作 Correlation ID。普通查询根入口也建立关联，业务实体的整个生命周期不自动共享同一关联。
- 关联进入 ExecutionContext，并由消息信封、Outbox 或需要恢复原操作的持久任务元数据保存；不要求给全部业务表增加关联列。业务上下文和消息保留既有非空约束，日志自动投影当前关联。认证前使用请求诊断状态，不为此伪造 Tenant/Actor。Trace 缺失不更换已有 correlation，二者都不建立身份或授权依据。
- 公网根入口生成 correlation，不采信调用方自定值；配置并保护的内部入口继承有效值，不靠请求自报 Header 判断内部来源。内部 HTTP 缺失/非法 correlation 时只补建一次并记录恢复 WARN；可信身份缺失仍按原契约拒绝。消息继续遵守既有必填与执行上下文契约，不能消费中另造 correlation 掩盖契约错误。HTTP 格式及旧消息读取边界见 Java 绑定。
- Outbox 保存不可变创建 Trace Context，以及当前发布代次的持久上下文；完整传播值包括 `traceparent` 和可选 `tracestate`，不能仅存 Trace ID。正常首发、自动重试、租约接管和宕机续投保持当前代次 Trace，每次实际执行使用新 Span。
- 人工重投原消息或显式原消息恢复任务建立新 Trace，以 Span Link 关联原创建上下文；新发布代次、Trace Context 与重投状态在同一数据库原子操作中保存，提交后才唤醒 Relay。自动重试继续这一代，不以等待时长、跨天或父 Span 已结束为由换 Trace。历史缺失/损坏上下文属于连续性无法保证的异常，修复后持久保存并复用，不能伪造旧上下文。
- 消息创建 Context 与本次投递 Context 分开。消费者保留本次有效消费 Context，将 creation 作为 Link，不用旧 creation 覆盖当前父链；同一批消息不共享虚构的 tenant、correlation 或父节点。Agent 可以用父子或 Link 表达投递关系，不要求所有业务相关执行强制归入一条 Trace。
- 重投原消息保持消息身份与已保存的关联，不能被重投管理请求自己的新关联覆盖。客户端独立再次提交默认形成新关联；若需认定为原操作，使用明确的幂等或流程记录恢复，不能只凭相同实体 ID 推断。`message_id`、直接因果标识及其他领域键保留独立含义，Trace 或关联标识不能替代消息去重身份。

以上是 2026-09-05 至 2026-09-06 确认的本仓库修订；决策经过见[Trace 与业务关联决策](decisions/TRACE-AND-BUSINESS-CORRELATION.md)，接入结构与验收责任见[整体接入方案](INTEGRATION-DESIGN.md)。规范已确认不表示运行接入已经完成。

本仓库首版直接采用 OTel SDK 接管路径，Java 使用 Agent。源快照及场景中标注“接入前”的自管 Trace 描述不构成本仓库实施路径：

- 使用 SDK 的标准 W3C 传播组件；禁止用正则或业务代码手写 `traceparent` 解析。合法上游 Context 按实际入口的 parent/Link 关系处理，保留适用的 flags 与 `tracestate`。
- 缺失/非法传播值交由标准组件放弃；若 Agent 已建立有效根 Context，框架直接复用，不额外再开一个根 Span。未采样的有效 Context 仍可用于日志关联。
- SDK 负责出站注入；普通线程切换传播完整当前 Context，不新建 Span。进程内新任务在提交前捕获父 Context，实际执行开始时创建任务 Span；独立定时触发新 Trace/关联，显式创建的持久任务保存触发关系，执行时新 Trace 并 Link 触发 Context。
...
HTTP 响应回写 `X-Correlation-Id` 与 `X-Trace-Id`；Problem 保留 `correlationId`，与本请求关联一致，不新增 Trace 正文字段或回写完整 `traceparent`。公网默认接受合法 W3C Trace Context，身份 baggage 不作为传播载体。未来若配置公网不采信 Trace，必须在 Agent 提取前的接缝实施。不可控链路使用领域键桥接，见 [TRACE-BREAK.md](decisions/TRACE-BREAK.md)。

## 4. 执行单元与调用分类

执行单元统一遵守：

1. 入口建立诊断状态，身份可信建立后才投影；退出时恢复进入前的上下文，包括本层写入键原本缺失的状态，不全量清除上层上下文。
2. 实际完成、失败或取消时记录一条 canonical INFO；异步请求不在首次 Filter 返回时提前记录，竞争终止回调只记录一次。最终日志保留本单元的 Trace 与可信身份快照，身份授权作用域仍及时退出。结果与耗时使用字段表达，不随成功失败升级级别。
...
2. 用 `trace_id` 查看当前 Trace；增加 `service.name` 查看单个服务。跨重放等多条 Trace 的业务操作，使用已定义的业务关联标识或领域键查询，并结合可用的 Span Link；单个 Trace 不保证覆盖完整业务历史。
...
- [NO-CUSTOM-CORRELATION.md](decisions/NO-CUSTOM-CORRELATION.md)：源规范的历史理由；其中“业务流用 trace_id 已经够”的概括已由第 3 节限定，不再作为禁止独立业务关联标识的依据。
- [TRACE-AND-BUSINESS-CORRELATION.md](decisions/TRACE-AND-BUSINESS-CORRELATION.md)：本仓库区分技术追踪与业务关联的已确认决定及证据责任。
- [TRACE-BREAK.md](decisions/TRACE-BREAK.md)：不可控链路 trace 断裂的已知局限。
- [INTEGRATION-DESIGN.md](INTEGRATION-DESIGN.md)：已确认的接入设计与验收责任；不表示已实现，日志活规则以本文为准。
