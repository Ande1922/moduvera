...
| `moduvera-web-spring-boot-starter` | native success bodies, validation, correlation and RFC 9457 errors | starter contract tests, Notes HTTP and microservice public black box |
...
| `moduvera-message-core` | transport-neutral Event/Async Command descriptors, typed Application message Handler with explicit `MessageId`, durable Outbox state machine, and committed Inbox precheck plus atomic handling | Handler unit contracts, PostgreSQL/MySQL store integration, and independent-parent Notes consumption from installed public artifacts |
| `moduvera-messaging-kafka-spring-boot-starter` | Cloud Stream imperative model, structured CloudEvents for Events, distinct tenant-only async-command envelope, logical route mapping, handler-bound context reconstruction and standard `Consumer<Message<byte[]>>` inbound assembly with an internal endpoint, synchronous broker ACK, JDBC Outbox/Inbox, bounded retry and Kafka DLQ integration | real Kafka Notes/App tests, provider-contract TCK, delivery-specific duplicate barriers, Order/Inventory duplicate-delivery ITs and microservice recovery black box; plaintext fixtures do not verify producer authentication or destination ACLs |
| `moduvera-test-support` | deterministic clocks, bounded eventually and run identifiers | test-scope consumers only |
| Runnable App image construction | all six `apps/` assemblies use one parameterized Dockerfile over Maven-built JARs, with Boot tools layers, a digest-pinned Java 26 JRE, root-owned read-only content, non-root execution and runtime-owned configuration | default five-App image build/inspection plus `catalog-app` health and HTTP smoke against real PostgreSQL; monolith image and default-port qualification are opt-in |
...
- `verification/reference-product/harness/verify.sh`: local acceptance harness that defaults to the microservice public HTTP contract, including concurrent tenant isolation and Kafka/App stop-recovery. Explicit monolith and dual-topology modes retain the shared contract for on-demand qualification. Its Compose assets are not production deployment guidance.
...
- `gateway-app`, `identity-app`, `app-monolith`: existing business-core composition reusing service-owned adapters, Local `CatalogApi` and Kafka Outbox/Inbox. Keep compilation and shared architecture checks; new iterations need not join or qualify this assembly. Monolith-specific integration, startup, black-box, recovery and image qualification are outside default acceptance. Historical dual-topology evidence remains historical; runtime support for a later target version requires explicit adaptation and fresh qualification under [ADR 0038](../adr/0038-retain-monolith-as-on-demand-assembly.md).
