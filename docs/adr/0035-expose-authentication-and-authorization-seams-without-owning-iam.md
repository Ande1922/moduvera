...
The scaffold owns the trusted authentication mapping, Execution Context, Actor and Tenant propagation, provider-owned Permission Code, fail-closed use-case authorization interface, stable denial/unavailable semantics, and Adapter verification contracts. It does not own a user directory, credential lifecycle, Tenant registry, RBAC tables, role or resource administration, Resource Manifest, License model, or object-level data authorization; consumer or external implementations map those models into the scaffold seams.
