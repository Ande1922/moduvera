...
Disposition: 接受。删除已经完成使命且无法作为 as-built 权威来源的 `DATA-MODEL-CONFIRMATION.md`，清理其陈旧引用；`OPEN-QUESTIONS.md` 只保留当前 primitive version 是否需要提升为 `AggregateVersion` 值对象这一尚待消费者证据的问题。改动已应用于工作区，等待未来提交后记录 `Fixed: <commit>`。
