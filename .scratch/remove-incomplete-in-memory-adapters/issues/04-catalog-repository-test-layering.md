...
- [ ] 缺失 Tenant Context 时的 fail-closed 行为以及跨租户不可见性通过最高有效生产 seam 验证。
- [ ] 当前内存 Product Repository 从生产源码和发布产物中移除，仓库内不再存在对它的引用。
- [ ] Catalog 聚焦测试、业务 Repository 兼容性测试及相关 App integration test 保持通过。
