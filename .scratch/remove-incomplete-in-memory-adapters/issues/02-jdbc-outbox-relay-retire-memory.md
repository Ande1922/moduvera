...
- [ ] 发送成功、可重试失败、不可重试失败、中断和未知 ACK 可以由测试源码内的窄脚本化 Message Transport 控制，而不引入正式 Broker fake。
- [ ] 当前内存 Outbox Store 与内存 Durable Publication 从生产源码和发布产物中移除，仓库内不再存在对它们的引用。
- [ ] 迁移后的容器测试按 `*IT` 运行，相关 Starter 与 Foundation 的聚焦验证保持通过。
