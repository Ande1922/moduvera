...
**租户标识（Tenant ID）**:
脚手架用于在身份、请求、消息、任务和基础设施之间关联同一租户的、不透明且可移植的字符串标识；当前产品契约为 1–64 个允许字符，并在 Reference Product 中按 `VARCHAR(64)` 持久化。该表示不规定正式业务系统的租户表主键类型；业务系统若采用 BIGINT，应在自身源码或明确的边界适配中作出选择，而不是由脚手架隐式转换。
_Avoid_: 租户表自增主键、带业务含义的租户编码、任意扩展字段容器
...
**库存预占决策（Inventory Reservation Decision）**:
Inventory 对一个预占命令的全部请求行作出的原子业务判断；结果只能是全部预占成功，或因一个以上商品不可用而整体拒绝。数据库锁、并发防护和命令幂等不是该决策本身。
_Avoid_: 库存扣减 SQL、预占事务、幂等记录
