...
assert receipt["committed"] - receipt["baseline"] == 12
...
assert (len(wire), len(transports), len(attempts), len(restored)) == (12, 13, 17, 13)
assert sorted(row["offset"] for row in wire) == list(range(12))
...
assert len(by_span) == len(spans) == 47
assert collections.Counter((span["name"], span["kind"]) for span in spans) == {
    ("fixture.publish", "INTERNAL"): 9,
    ("inbound-causality publish", "PRODUCER"): 9,
    ("inbound-causality process", "CONSUMER"): 11,
    ("mq.process", "INTERNAL"): 16,
...
assert len(canonical) == 17 and len(warnings) == 4 and len(errors) == 2
assert collections.Counter(event["event"]["outcome"] for event in canonical) == {"success": 11, "failure": 6}
...
for event in errors:
    ...
    assert original["message_id"] in ("terminal", "exhausted")
...
for span in dlq:
    ...
    final = next(event for event in errors if event["trace_id"] == span["traceId"])
...
print(json.dumps({"brokerDeliveries": len(wire), "directFallbackExecutions": 1,
                  "actualAttempts": len(canonical), "retryDecisions": len(warnings),
                  "finalErrors": len(errors), "sampledProcessingSpans": 16,
                  "unsampledAttempts": 1, "agentConsumerSpans": 11, "originalBinderDlqSends": 2,
