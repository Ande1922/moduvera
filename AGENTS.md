...
- For an asynchronous-only capability, publish provider-owned, versioned command and event records in the provider's API module, and let the message Inbound Adapter decode or map them and invoke an independent typed Application Handler. Add a Java `*Api` method only when a supported direct local or remote call exists; a message Handler alone does not justify a synchronous interface.
