...
def main() -> int:
    ...
    try:
        ...
        probe(
            ...
            ["/usr/bin/head", "-c", "1", str(args.repository_root.resolve() / "benchmarks/store-saas-persistence-model/PROTOCOL.md")],
            False,
        )
        probe(
            "evaluator_read_denied",
            ["/usr/bin/head", "-c", "1", str(args.repository_root.resolve() / "benchmarks/store-saas-persistence-model/pilot/t01/evaluator/scripts/scan_sql_scope.py")],
            False,
        )
